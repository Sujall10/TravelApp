...
flight_oneway = pd.read_excel('notebooks\\OneWay_by_Hetvi.xlsx', engine='openpyxl')  # Replace with your flight dataset file path
...
for i, row in flight_oneway.iterrows():
    cursor.execute("""
        INSERT INTO flight_OneWay (travelcode, user_id, departure, arrival, flight_number, flight_type, 
                            flight_duration, flight_distance, flight_agency, departure_date, flight_price)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
    """, (row['travelCode'], row['User_ID'], row['Departure'], row['Arrival'], row['flight_number'], row['flightType'], 
          row['Flight Duration (hh:mm)'], row['Flight Distance (km)'], row['Flight_agency'], row['Departure DateTime'], row['Calculated_Flight_Price']))
